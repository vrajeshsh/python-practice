...
class OrderedList():
    ...
    def merge_ordered(self, ord_list):
        i, j, obj = 0, ord_list.size - 1, OrderedList(self.size+ord_list.size)
        while i < self.size and j >= 0:
            if self.ordered_list[i] <= ord_list.ordered_list[j]:
                obj.ordered_list.append(self.ordered_list[i])
                i+=1
            else:
                obj.ordered_list.append(ord_list.ordered_list[j])
                j-=1
        obj.ordered_list+=self.ordered_list[i:]
        # while j>=0: #Everything OKAY, Just get rid of this loop by an in-built function
        #     obj.ordered_list.append(ord_list.ordered_list[j])
        #     j-=1
        obj.ordered_list.append(ord_list.ordered_list[j::-1])
        return obj
